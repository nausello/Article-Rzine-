...
Un des enjeux de l'analyse spatiale des phénomènes sociaux repose sur la capacité à constituer des unités territoriales cohérentes, à la fois homogènes en termes de caractéristiques internes et pertinentes au regard de la thématique abordée. Ce défi de la délimitation territoriale, généralement désigné par le terme MAUP (*Modifiable Areal Unit Problem*) ou problème d'agrégation spatiale, est au cœur d'une littérature en géographie quantitative [@Openshaw1984; @Fotheringham2000]. Deux logiques se distinguent. 

La première, celle de la régionalisation contrainte, part des observations individuelles géolocalisées et reconstruit une partition de l'espace *ex nihilo*, sans présupposer aucun découpage préexistant. Le principe consiste à regrouper les unités spatiales élémentaires en zones agrégées de manière à maximiser l'homogénéité interne de chaque zone (au regard des variables considérées) tout en respectant une contrainte de contiguïté géographique, c'est-à-dire en n'agrégeant que des unités voisines les unes des autres. Cette double exigence, de cohésion statistique et de continuité spatiale, distingue ces méthodes d'une simple classification statistique (de type k-*means*), qui ignore la localisation des observations et peut ainsi produire des groupes discontinus dans l'espace. Plusieurs familles d'algorithmes ont été développées pour résoudre ce problème d'optimisation : la classification hiérarchique géographiquement contrainte [@Chavent2018], de l'algorithme SKATER [@Assuncao2006] ou des approches REDCAP [@Guo2008]. 

La seconde logique, moins outillée, ne cherche pas à créer un découpage mais à évaluer la pertinence d'un découpage existant [@Coombes1986; @Farmer2011; @Klapka2013] : dans quelle mesure un découpage institutionnel (EPCI, arrondissement, zone d'emploi) reflète-t-il une réalité socio-spatiale homogène, ou agrège-t-il au contraire des unités dont les profils seraient statistiquement plus proches d'un groupement voisin [@Openshaw1984; @Briant2015] ? 

La distinction entre régions **fonctionnelles** et régions **administratives** structure une partie des travaux en géographie quantitative [@Openshaw1984; @Farmer2011], et trouve une illustration empirique dans les nombreux effets de frontière documentés en fiscalité [@Revelli2001], sur les marchés immobiliers [@Guerois2009; @Fack2010] ou dans la concurrence fiscale entre collectivités [@Tiebout1956].

Cet article propose une présentation du package R `spatialRegroup` d'évaluation et de correction marginale de découpages géographiques existants disponible sur le [CRAN](https://cran.r-project.org/web/packages/spatialRegroup/index.html). Contrairement aux méthodes de régionalisation globale, `spatialRegroup` ne reconstruit pas une partition de zéro : il part d'un découpage géographique donné et identifie les seules unités frontalières dont le profil attributaire est statistiquement plus proche d'un groupe voisin que de leur propre groupe d'appartenance, sur la base de distances euclidiennes calculées dans leur voisinage immédiat.Cette approche locale permet de corriger à la marge les incohérences d'un zonage sans en altérer la structure d'ensemble [@Openshaw1984], tout en restant utilisable en complément des méthodes de reconstruction globale plutôt qu'en remplacement. 

Les variables attributaires mobilisées pour ce calcul d'affinité peuvent être de simples caractéristiques socio-économiques, mais aussi, et c'est un usage que nous illustrons dans cet article, des coefficients issus d'une régression géographiquement pondérée multiscalaire (MGWR) [@Fotheringham2017; @Yu2020]. La MGWR permet en effet d'estimer, commune par commune, des coefficients locaux qui rendent compte de l'hétérogénéité spatiale des relations statistiques [@Brunsdon1996; @Fotheringham2002], une hétérogénéité que les seules caractéristiques brutes des unités ne peuvent capturer. Utilisés comme variables d'entrée de `spatialRegroup`, ces coefficients permettent ainsi de fonder la reclassification non plus sur ce que sont les unités, mais sur la manière dont elles se comportent statistiquement vis-à-vis d'un phénomène donné.

Nous illustrons les capacités du package à travers une application à la taxe foncière sur les propriétés bâties (TFPB) en région Île-de-France sur l'année 2024. La TFPB constitue un cas d'application particulièrement pertinent puisque son taux résulte d'une superposition de décisions fiscales communales et intercommunales : l'appartenance d'une commune à un EPCI plutôt qu'un autre produit des effets directs capitalisés dans les prix immobiliers [@Fack2010; @Revelli2001]. Identifier les communes dont le profil fiscal est statistiquement incohérent avec leur EPCI d'appartenance revient ainsi à localiser des effets de frontières au niveau intercommunal.

```{r echo=T, warning=F, results=F, message=F}

# Packages nécessaires à la reproduction de la chaîne de traitement

# Chargement et manipulation des données
library(tidyverse)       # Suite pour la manipulation et la visualisation de données
library(magrittr)        # Opérateurs (%>%)
library(purrr)           # Programmation fonctionnelle

# Données spatiales
library(sf)              # Manipulation de données spatiales vectorielles
library(sp)              # Classes spatiales (requis par GWmodel)

# Analyse spatiale et modélisation statistique
library(spdep)           # Autocorrélation spatiale et matrices de voisinage
library(GWmodel)         # Régression géographiquement pondérée (GWR, MGWR)
library(rgeoda)          # Algorithmes de régionalisation contrainte (SKATER, REDCAP, Max-P)
library(lme4)            # Modèles mixtes (effets fixes et aléatoires)
library(spatialRegroup)  # Regroupement spatial local (package présenté dans cet article)

# Visualisation analytique
library(ggplot2)         # Création de graphiques
library(scales)          # Formatage des axes et des échelles
library(ggrepel)         # Étiquettes de texte sans chevauchement
library(ggpubr)          # Extraction et assemblage de légendes ggplot
library(patchwork)       # Assemblage de graphiques ggplot
library(tmap)            # Cartographie thématique

# Packages utilisés pour la mise en page de l'article 

library(kableExtra)      # Mise en forme des tableaux HTML
library(sjPlot)          # Tableaux de résultats de modèles HTML (tab_model)
```
...
reclasse_boxes <- coords_g |>
  as.data.frame() |>
  mutate(id = grille$id) |>
  dplyr::filter(id %in% dest_colors$id) |>
...
ggsave(
  ...
  plot     = planche_ok & theme(plot.margin = margin(0, 0, 0, 0)),
  width    = 9,
  height   = 5,
  dpi      = 300,
  bg       = "white"
...
for (nom in names(fichiers_shp)) {
  ...
  if (!file.exists("data/communes.shp")) {
...
Ces quatre sources sont agrégées à l'échelon communal pour constituer la base de travail finale (`df_final`), téléchargeable ci-dessous. Le code ci-après documente l'intégralité de la chaîne de prétraitement ayant permis de construire ce fichier — téléchargement des données brutes depuis Zenodo, nettoyage des DVF et jointures finales. Il n'est pas ré-exécuté au rendu de l'article (`eval=FALSE`) car `df_final.rds` est chargé directement via `readRDS()`.
...
# Téléchargement des données brutes (Zenodo)
...
for (nom in names(fichiers)) {
  ...
  if (!file.exists(dest)) download.file(fichiers[[nom]], destfile = dest, mode = "wb")
}

# Fond de carte communes (shapefile)
communes_zip <- "data/shp/communes.zip"
if (!file.exists(communes_zip)) {
  download.file(
    "https://zenodo.org/records/21307967/files/communes.zip",
    destfile = communes_zip, mode = "wb"
  )
  unzip(communes_zip, exdir = "data")
}
...
com          <- sf::read_sf("data/communes.shp")

# Nettoyage et agrégation des DVF (2021-2024)

nettoyer_dvf <- function(DVF) {
  DVF |>
    dplyr::filter(nature_mutation == "Vente",
    ...
    mutate(across(c(prix, surface, piece), as.numeric)) |>
    dplyr::filter(!is.na(prix)) |>
    group_by(id) |> dplyr::filter(n() == 1) |> ungroup() |>
    dplyr::filter(dplyr::between(prix, 15000, 5000000)) |>
    dplyr::filter(
      case_when(type == "Appartement" ~ dplyr::between(surface, 10, 200),
                type == "Maison"      ~ dplyr::between(surface, 10, 350),
                TRUE                  ~ FALSE)
    ) |>
    mutate(prixm2 = prix / surface) |>
    dplyr::filter(dplyr::between(prixm2, 330, 10000))
}

datasets      <- list(dvf21 = dvf21, dvf22 = dvf22, dvf23 = dvf23, dvf24 = dvf24)
dvf_nettoyes  <- lapply(datasets, nettoyer_dvf)

dvf_communes <- lapply(dvf_nettoyes, function(dvf) {
  dvf |>
    group_by(codecommune) |>
    summarise(nb_ventes = n(), prixm2 = mean(prixm2, na.rm = TRUE), .groups = "drop")
})

# Construction du fichier de travail final

# Moyenne des équipements 2019-2024 (BPE)
bpe1 <- bpe |>
  group_by(GEO, TIME_PERIOD) |>
  summarise(nb_equipements = sum(OBS_VALUE, na.rm = TRUE), .groups = "drop") |>
  pivot_wider(names_from = TIME_PERIOD, values_from = nb_equipements, values_fill = 0) |>
  mutate(moy_equip = (`2019` + `2024`) / 2)

# Codes EPCI avec intégration des EPT franciliens
epci_com_idf <- epci_com_idf |>
  left_join(ept_idf |> dplyr::select(Code.Officiel.Commune, Code.Officiel.EPT),
            by = "Code.Officiel.Commune") |>
  mutate(`Code Officiel EPCI` = if_else(!is.na(Code.Officiel.EPT),
                                         Code.Officiel.EPT, Code.Officiel.EPCI)) |>
  dplyr::select(-Code.Officiel.EPT)

# Taux TFPB moyens par commune
taux_moyen <- com |>
  sf::st_drop_geometry() |>
  left_join(fisc, by = c("DCOE_C_COD" = "INSEE.COM")) |>
  group_by(DCOE_C_COD) |>
  summarise(
...
# Base spatiale : communes + revenus + taux TFPB
df_reduit <- com |>
  left_join(rev_21,     by = c("DCOE_C_COD" = "Code")) |>
  left_join(taux_moyen, by = "DCOE_C_COD") |>
  sf::st_make_valid() |>
  distinct(DCOE_C_COD, .keep_all = TRUE) |>
  dplyr::select(DCOE_C_COD, median, Taux_TFB_2021, Taux_TFB_2022,
                Taux_TFB_2023, Taux_TFB_2024, Taux_TFB_moy,
                EPCI_CODE, REGION_COD, DDEP_C_COD, geometry)

# Jointure des prix DVF par année
annees <- c("dvf21" = "2021", "dvf22" = "2022", "dvf23" = "2023", "dvf24" = "2024")
df_final <- df_reduit
for (key in names(annees)) {
  df_final <- df_final |>
    left_join(
      dvf_communes[[key]] |>
        rename_with(~ paste0(., "_", annees[[key]]), c("prixm2", "nb_ventes")),
      by = c("DCOE_C_COD" = "codecommune")
    )
}
...
df_final <- df_final |>
  mutate(
    prix_m2_moy = rowMeans(
      cbind(prixm2_2021, prixm2_2022, prixm2_2023, prixm2_2024), na.rm = TRUE),
    nb_ventes_total = rowSums(
      cbind(nb_ventes_2021, nb_ventes_2022, nb_ventes_2023, nb_ventes_2024), na.rm = TRUE)
  ) |>
  dplyr::filter(!is.na(prix_m2_moy))

# Intégration des codes EPCI/EPT, équipements, exclusion Paris et NA revenus
epci_com_idf$Code.Officiel.Commune <- as.character(epci_com_idf$Code.Officiel.Commune)

df_final <- df_final |>
  rename(codecommune = DCOE_C_COD) |>
  left_join(epci_com_idf, by = c("codecommune" = "Code.Officiel.Commune")) |>
  rename(EPCI = `Code Officiel EPCI`) |>
  mutate(EPCI = as.factor(EPCI)) |>
  left_join(bpe1, by = c("codecommune" = "GEO")) |>
  dplyr::filter(!startsWith(DDEP_C_COD, "75")) |>
  dplyr::filter(!is.na(median))

saveRDS(df_final, file = "data/df_final.rds")
```
...
sp_long <- sp_sf %>%
  dplyr::select(all_of(vars_beta), geometry) %>%
  pivot_longer(
...
corre1 <- ggplot(results_grid, aes(x = n_reclass, y = delta_aic)) +
  ...
  geom_label_repel(
    data          = results_grid |> dplyr::filter(abs(threshold - 0.12) > 1e-9),
...
# Objet allégé pour tmap uniquement
df_final_2_map <- df_final_2 |>
  dplyr::select(EPCI, EPCI_final, geometry)

# Filtrer les communes reclassées
reclassees <- res_avance |>
  dplyr::filter(EPCI != EPCI_regroup) |>
  dplyr::select(EPCI, EPCI_regroup, geometry)
 
# Carte 1 : EPCI originaux
tm1 <- tm_shape(df_final_2_map) +
  tm_polygons(
    col = "EPCI",
    palette = "Set3",
    border.col = "grey",
